    ...
    function show_timetable_options() {
        ...
        for (day_object of config.data.timetable) {            
            ...
            for (var i = 0; i < config.data.periods.length; i++) {
                try {
                    add_period_input_group(day_object.schedule[i].subject, day_object.schedule[i].teacher);
                }
                catch (TypeError) {
        ...
        function add_period_input_group(subject, teacher) {
...
function save() {
    ...
    for (var i = 0; i < day_name_inputs.length; i++) {
        ...
        for (input_group of schedule_input_groups) {
            ...
            var teacher = input_group.children[1].value;
